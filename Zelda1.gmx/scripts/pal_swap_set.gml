...
if(shader_is_compiled(_shader) && _pal>0)
{
    ...
    ds_list_copy(_colors,ds_list_find_value(_palettes,_pal)); //Copy list of colors.
    
    //Overwrite colors
    var _iteration=1;
    while(ds_grid_value_exists(Pal_Override_Grid,0,0,0,ds_grid_height(Pal_Override_Grid)-1,id))
    {   //Iterate through all the overrided colors and replace them.
        //show_debug_message("replacing color: "+string(_iteration));
        var _row=ds_grid_value_y(Pal_Override_Grid,0,0,0,ds_grid_height(Pal_Override_Grid)-1,id);
        var _index=Pal_Override_Grid[# 1,_row];
        var _color=Pal_Override_Grid[# 2,_row];
        Pal_Override_Grid=ds_grid_delete_row(Pal_Override_Grid,_row);
        ds_list_replace(_colors,_index,_color);
        _iteration++;
    }
    ...
    ds_list_destroy(_colors);//Cleanup List
